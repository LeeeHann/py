...
class Learning(): 
    ...
    def run(self):
        ...
        # self.background=tkinter.PhotoImage(file="Image/images.gif")
        # self.bg=tkinter.Label(self.root,image=self.background)
    ...
    def questions_window(self, question):
        self.button1 = tkinter.Button(text = "答え",font = ("times New Roman", 30), command = self.clickBtn)
    ...
    def correctAnswer(self):
        ...
        if self.number == 5:
            ...
            self.button2.place(x = 650, y = 650)
        
    def incorrectAnswer(self, correctAnswer):
        self.label3 = tkinter.Label(self.root, text = "不正解\n>:<", font=("times New Roman", 60))
        self.label3.place(x = 610, y = 200)
        self.label4 = tkinter.Label(self.root, text = f"正解の答え: {correctAnswer}", font=("System", 50))
        self.label4.place(x = 500, y = 400)
        if self.number == 5:
            currentWindow = "incorrect"
            self.button6 = tkinter.Button(text = "score", font=("times New Roman", 30),command = lambda: self.scoreBtn(currentWindow))
            self.button6.place(x = 650, y = 600)
        else:
            self.button3 = tkinter.Button(text = "次へ", font=("times New Roman", 30),command = self.incorrectBtn)
            self.button3.place(x = 650, y = 650)
